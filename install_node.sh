...
${bold}${NODENAME} is no longer actively supported!${normal}
   ...
   * ${green}https://deb.nodesource.com/setup_15.x â€” Node.js 15 \"Fifteen\"${normal}
...
if [ -x /usr/bin/curl ]; then
    exec_cmd_nobail "sudo curl -sLf -o /dev/null 'https://deb.nodesource.com/${NODEREPO}/dists/${DISTRO}/Release'"
    RC=$?
else
...
if [ -x /usr/bin/curl ]; then
    exec_cmd "sudo curl -s $node_key_url | gpg --dearmor | tee $local_node_key >/dev/null"
else
...
## To install the Yarn package manager, run:
     sudo curl -sL $yarn_key_url | gpg --dearmor | sudo tee $local_yarn_key >/dev/null
