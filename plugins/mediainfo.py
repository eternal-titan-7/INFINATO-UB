...
async def mi(e):
    ...
    url = make_html_telegraph("Mediainfo", "INFINATO", f"<code>{murl}</code>")
    ...
    if hasattr(r.media, "document"):
        ...
        naam = await infinato_bot.download_media(r.media)
    out, er = await bash(f"mediainfo '{naam}' --Output=HTML")
    urll = make_html_telegraph("Mediainfo", "INFINATO", out)
    if er:
