...
async def igif(e):
    ...
    try:
        await bash(f'ffmpeg -i "{z}" -vf format=gray inf.gif -y')
        await e.client.send_file(e.chat_id, "inf.gif", support_stream=True)
        os.remove(z)
        os.remove("inf.gif")
        await xx.delete()
...
async def igif(e):
    ...
    try:
        await bash(
            f'ffmpeg -i "{z}" -vf lutyuv="y=negval:u=negval:v=negval" inf.gif -y'
        )
        await e.client.send_file(e.chat_id, "inf.gif", support_stream=True)
        os.remove(z)
        os.remove("inf.gif")
...
async def vtogif(e):
    ...
    if int(dur) < 120:
        z = await ultroid_bot.download_media(a.media)
        await bash(
            f'ffmpeg -i {z} -vf "fps=10,scale=320:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" -loop 0 inf.gif -y'
        )
        await e.client.send_file(e.chat_id, "inf.gif", support_stream=True)
        os.remove(z)
        os.remove("inf.gif")
        ...
        z = vid.name
        await bash(
            f'ffmpeg -ss 3 -t 100 -i {z} -vf "fps=10,scale=320:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" -loop 0 inf.gif'
        )
        await e.client.send_file(e.chat_id, "inf.gif", support_stream=True)
        os.remove(z)
        os.remove("inf.gif")
